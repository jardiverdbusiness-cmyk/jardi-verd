...
export function Hero() {
  ...
  return (
    <section className="relative overflow-hidden bg-forest-900 text-cream-50">
      <div
        aria-hidden
        className="animate-float pointer-events-none absolute -right-24 -top-24 h-96 w-96 rounded-full bg-forest-700/40 blur-3xl"
      />
      <div
        aria-hidden
        className="animate-float-slow pointer-events-none absolute -bottom-32 left-1/3 h-96 w-96 rounded-full bg-gold-500/10 blur-3xl"
      />
            ...
            {(["stat1", "stat2", "stat3"] as const).map((key) => (
              <div key={key}>
                ...
                <dd className="font-display text-2xl font-semibold text-gold-400 sm:text-3xl">
                  <AnimatedCounter value={t(`${key}Value`)} />
                </dd>
